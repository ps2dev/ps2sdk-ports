...
git clone --depth 1 -b v1.3.3 https://github.com/ps2dev/gsKit || { exit 1; } 
...
PROC_NR=1 "${MAKECMD}" -C gsKit -j "$PROC_NR" || { exit 1; }
PROC_NR=1 "${MAKECMD}" -C gsKit -j "$PROC_NR" install || { exit 1; }
