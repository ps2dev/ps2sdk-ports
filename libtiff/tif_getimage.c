 ...
 */
int
TIFFReadPS2Image(TIFF* tif, uint32 rwidth, uint32 rheight, uint32* raster, int stop)
{
    char emsg[1024] = "";
    TIFFRGBAImage img;
    int ok;
    ...
    for (y = (img.height - 1), cy = 0; y >= 0; y--, cy++)
    {
        ...
        {
            ((uint8*)raster)[(y*img.width+x)*4+0] = ((uint8*)nraster)[(cy*img.width+x)*4+0];
            ((uint8*)raster)[(y*img.width+x)*4+1] = ((uint8*)nraster)[(cy*img.width+x)*4+1];
            ((uint8*)raster)[(y*img.width+x)*4+2] = ((uint8*)nraster)[(cy*img.width+x)*4+2];
            tmp = (((uint8*)nraster)[(cy*img.width+x)*4+3] * 128);
            ((uint8*)raster)[(y*img.width+x)*4+3] = tmp / 255;
        }
    }
    free(nraster);
